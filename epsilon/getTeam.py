...
# EP-2: Display Team
def getTeam(tid, dao):
    q = "With temp as (Select Users.uid, Users.name, Users.contact,Users.rid, Roles.role_type from Users inner join Roles on Users.rid=Roles.rid) Select temp.name, temp.type, temp.contact, temp.uid, Teams.tid,temp.rid from temp, Teams where Teams.uid=temp.uid and Teams.tid="+str(tid)
